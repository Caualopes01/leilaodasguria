...
export default function TenantFooterNav() {
  ...
  const storageKey = `leilao_notif_count_${tenantSlug}`
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`tenant-notif-${tenantSlug}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'lances',
          ...
          toast.custom((t) => (
            <div className="bg-white rounded-2xl shadow-2xl border border-green-100 p-4 w-[340px] max-w-[90vw] flex flex-col gap-3 mx-auto">
              ...
              <button
                onClick={() => {
                  toast.dismiss(t)
                  router.push(`/loja/${tenantSlug}/leilao/${novoProduto.slug}`)
                }}
  ...
  const tabs = [
    {
      href: `${basePath}/leiloes`,
      label: 'Explorar',
      icon: ShoppingBag,
      active: pathname === `${basePath}/leiloes` || pathname.startsWith(`${basePath}/leilao`),
    },
    {
      href: `${basePath}/meus-lances`,
      label: 'Meus Lances',
      icon: Bell,
      active: pathname === `${basePath}/meus-lances`,
    },
    {
      href: `${basePath}/novidades`,
      label: 'Novidades',
      icon: Gavel,
      active: pathname === `${basePath}/novidades`,
