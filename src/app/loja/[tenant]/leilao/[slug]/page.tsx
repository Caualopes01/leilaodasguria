...
export default function TenantLeilaoPage() {
  ...
  const isCritical = timeLeft.total > 0 && timeLeft.total < 60000
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`tenant-lances-${produto.id}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'lances',
  ...
  async function handleSubmitLance() {
    ...
    if (latestBids && latestBids.length > 0) {
      ...
      const currentMinVal = currentMax + (produto.incremento_minimo || 1)
    ...
    // Buscar tenant_id para inserir no lance
    const { data: tenant } = await supabase
      .from('tenants')
      .select('id')
      .eq('slug', tenantSlug)
    ...
    const participoStr = localStorage.getItem(`leilao_produtos_ids_${tenantSlug}`)
    const participando = participoStr ? JSON.parse(participoStr) : []
    if (!participando.includes(produto.id)) {
      participando.push(produto.id)
      localStorage.setItem(`leilao_produtos_ids_${tenantSlug}`, JSON.stringify(participando))
    }
    ...
    <div className="min-h-screen bg-white max-w-lg mx-auto pb-24 overflow-x-hidden">
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-rosa-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={`/loja/${tenantSlug}/leiloes`} className="p-2 -ml-2 rounded-full hover:bg-rosa-50 text-gray-500 transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div className="flex items-center gap-1.5">
            <Heart className="w-4 h-4 text-rosa-500" fill="currentColor" />
            <span className="font-display font-bold text-rosa-600 text-sm">{tenantNome || 'Leilão'}</span>
          </div>
        </div>
        <button
          ...
          <div className="bg-rosa-50 border border-rosa-200 rounded-2xl p-6 text-center shadow-sm animate-fade-in relative overflow-hidden">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-3 border border-rosa-100/50 flex items-center justify-center gap-2">
              ...
              <p className="text-xs text-gray-600 font-medium">
                Entraremos em contato com a vencedora.
              </p>
            </div>
          </div>
        )}
