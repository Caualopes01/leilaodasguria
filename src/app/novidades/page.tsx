...
function ProdutoCard({ produto }: { produto: Produto }) {
  ...
  return (
    <Link href={`/leilao/${produto.slug}`} className="block">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md hover:border-rosa-200 transition-all active:scale-[0.98] p-3 flex items-center gap-3">
        {/* Imagem */}
        <div className="w-[84px] h-[84px] rounded-xl bg-rosa-50 overflow-hidden flex-shrink-0 relative">
          ...
          <div className="absolute top-1 left-1 bg-green-500/90 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase shadow-sm">Novo</div>
        </div>
        
        {/* Info & Price */}
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <h3 className="font-semibold text-gray-900 text-sm line-clamp-1 leading-tight mb-1">{produto.titulo}</h3>
          
          <div className="flex items-center gap-1.5 mb-1 text-[11px] font-medium text-gray-500">
            <Clock className="w-3 h-3 text-rosa-500" />
            <span>Fim em {timer}</span>
          </div>

          <div className="flex items-baseline gap-1 mt-0.5">
            <span className="text-[10px] text-gray-400 uppercase font-semibold tracking-wider">Lance</span>
        ...
        <div className="flex-shrink-0">
          <div className="flex items-center justify-center bg-rosa-50 text-rosa-600 hover:bg-rosa-100 font-bold text-[10px] px-3 py-2 rounded-xl transition-colors gap-1 uppercase tracking-wide">
            Ver <ChevronRight className="w-3 h-3 -mr-1" />
          </div>
        </div>
      </div>
    </Link>
