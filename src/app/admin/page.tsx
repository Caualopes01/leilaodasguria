            ...
            {produtos.filter(p => p.status === 'ativo').map(p => (
              <div key={p.id} className="flex items-center gap-4 p-4 rounded-xl border border-green-100 bg-green-50">
                ...
                <div className="flex items-center gap-2">
                  <Link
                    href={`/admin/produtos/${p.id}/lances`}
                    className={`flex items-center justify-center gap-1 text-xs font-semibold px-3 py-2 rounded-xl border transition-colors ${
                      (p as any).lances?.length 
                        ? 'border-rosa-200 text-rosa-700 hover:bg-rosa-50 hover:border-rosa-300' 
                        : 'border-gray-200 text-gray-500 bg-gray-50 cursor-not-allowed opacity-70'
                    }`}
                    onClick={(e) => {
                  ...
                  <Link
                    href={`/leilao/${p.slug}`}
                    target="_blank"
                    className="flex items-center justify-center gap-1 text-xs font-semibold px-3 py-2 rounded-xl bg-gray-50 text-gray-600 hover:bg-gray-100 transition-colors"
                  >
